...
- **Issue-driven workflow**: Documented the GitHub CLI loop (`gh issue develop` → `Closes #N` → board auto-Done) in `spec/standards.md` and `CLAUDE.md`. Trimmed `spec/roadmap.md` to forward-looking themes and annotated `gap-analysis.md` / `ux-review.md` as point-in-time snapshots, now that the granular backlog lives in GitHub issues.
