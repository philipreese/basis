...
from backend.models import PositionSchema, PortfolioConfigSchema, MarketStateSchema, OptionLegSchema
...
def run_lifecycle_scan(
    ...
    if current_regime == "TRENDING_BEAR":
        ...
        elif len(position.legs) == 1 and position.legs[0].option_type == "CALL" and position.legs[0].direction == "LONG":
            conflict = True
        ...
        if strategy == "IRON_CONDOR":
            short_put_strike = next((l.strike for l in position.legs if l.option_type == "PUT" and l.direction == "SHORT"), None)
            short_call_strike = next((l.strike for l in position.legs if l.option_type == "CALL" and l.direction == "SHORT"), None)
            ...
            if short_put_strike and short_call_strike:
                ...
                if breached_put:
                    conflict = True
                    conflict_desc = f"Iron Condor short put strike {short_put_strike} breached within 2% (SPY: {spy_price})."
                elif breached_call:
                    conflict = True
                    conflict_desc = f"Iron Condor short call strike {short_call_strike} breached within 2% (SPY: {spy_price})."

    elif current_regime == "EVENT_CATALYST":
