...
4. **Verify**: Run `./scripts/verify-project.ps1` and confirm tests pass before declaring done. The git hooks split the same script in two (#988, #997): **pre-commit runs `pixi run lint` only** (seconds, so a single `git commit` fits an agent's command ceiling), and **pre-push runs `pixi run test-backend`** (plus `test-frontend` when the pushed commits touch `frontend/`), with secrets scan scoped to pushed files and redundant warning-only workflow checks skipped; the blocking main/master branch guard still runs (#997). A commit with a failing test is therefore accepted at commit and refused at push; CI runs the full unscoped suite on the PR. A third hook, **`commit-msg`, strips AI-attribution trailers** (#1016) — see "No AI attribution" below. `pixi run install-hooks` installs all three; `pixi run verify-hook-selftest` pins this behaviour against a throwaway repo.
...
*   **No AI attribution**: Commit messages and PR bodies contain no AI/assistant attribution, session links, or generated-by footers of any kind. **Enforced, not requested** (#1016): the `commit-msg` hook strips them from every commit message, because an agent's own harness instructs it to add them and those instructions live outside this repo — they cannot see this rule. A `Co-authored-by` naming a *human* is a real credit and is never touched. PR bodies have no hook and remain yours to check.
