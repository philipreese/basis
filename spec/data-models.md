...
interface PlaybookDefinition {
  ...
  entry_filters: {
    min_ivr: number;   // seeded 0.0 EVERYWHERE (#1035) — see the note below on why a floor here is meaningless
    max_ivr: number;
    min_vrp: number | null;  // required vol points of VIX − SPY RV20; null = no VRP gate (#1035)
...
- **Per-underlying telemetry** (#139): `MarketStateSchema` carries `underlying_prices` / `underlying_sma20` dicts, computed by the executor from `index_history` at scan time (never persisted). Scan lookups go through a telemetry-proxy map (`XSP → SPY`); non-SPY underlyings (IWM, GLD) also get an RV20 percentile-rank pseudo-IVR published into `underlying_ivrs` so the rank windows and gates work unchanged, and SPY's own entry in the persisted `underlying_ivrs` is the same rank written by the nightly refresh (#989) — the one statement of the IVR source per underlying is in [domain-rules.md](domain-rules.md) → "Playbook matching (Layer C)". An underlying with insufficient history is absent from the dicts, and its playbooks are suppressed — the scan never derives strikes off SPY telemetry for a different-scale asset.
- Book configs are versioned and hashed (edit ⇒ new `config_version` + hash); the Live Gate attaches to a `(book_id, config_hash)` pair — the multi-book extension of [ADR-0003](decisions.md#adr-0003--playbook-snapshot-immutability). **`backend/seeds.py` is the only source of truth for `books.config`** ([ADR-0013](decisions.md#adr-0013--seedspy-is-the-single-source-of-truth-for-book-configs-no-out-of-band-db-edits)) — `database.init_db` reverts any stored config that doesn't hash-match its seed on every process start, so a direct DB edit is prohibited and futile; change `seeds.py` instead and let the sync propagate it.
- **Book allocation** ([ADR-0009](decisions.md#adr-0009--accelerated-experiment-matrix) and its amendments, superseding the 2026-08-18 six-book plan; `backend/seeds.py` is the current source of truth for book count): an experiment matrix, one question per book — B01–B06 core V0/V1/V2 × XSP/SPY grid plus single-variable arms and controls. Book `config` gains optional keys `playbook_ids` (whitelist), `playbook_overrides` (dot-keyed field overrides, e.g. `"execution_specs.target_dte": 24`, revalidated through `PlaybookDefinitionSchema`), `ignore_regime`, `ignore_ivr`, `exit_on_regime_flip` (B28's exit-side experiment, #254), `require_consensus` (B29, #316), `roll_time_exits` (B31, #318), `dedup_playbook_entries` (B32, #411), `delta_cap_vix` (B33, #816 — vol-aware short-leg delta cap for credit structures, `min(target, cap/VIX)`, fail-closed sit-out on a missing VIX close), and `min_credit_ratio` (B34, #820 — minimum-credit floor: a CREDIT entry with `|net_mid| < ratio × width_bound` is refused, `ENTRY_REFUSED_THIN_CREDIT`; debit structures never checked; zero width bound leaves the floor inert); all participate in `config_hash`. B09/B10 landed with multi-underlying telemetry (#139), B19/B20 with the V3 variant (#134), B18 with the broken-wing butterfly (#132), B21 with calendar spreads (#133), B22 with TLT telemetry after the ex-div defense (#135, #130 — monthly dividends make B22 put-side by construction), and B23–B27 as the 3-point knob sweeps (20Δ/40Δ spreads-only, 52-DTE, 75% profit take, $2 wings — #222) with B28's regime-flip exit closing the set.
