...
**Decision.** Autonomy advances through three strictly ordered levels (terms in [CONTEXT.md](../CONTEXT.md)): **Operator** (agent runs the evening pipeline on a schedule, human places orders), **Executor (Paper)** (agent places orders in a paper account, fully autonomous within the hard blocks), **Executor (Live)** (agent trades the real IRA, approval-per-trade at first). Promotion from Paper to Live requires the **Live Gate**: ≥30 closed paper trades AND ≥3 calendar months AND zero hard-block/gate breaches AND expectancy ≥ 0 after a slippage haircut (paper fills run optimistic). The **risk envelope** — the **lab book envelope cap** (`Envelope.max_deployed_pct`, `backend/book_gates.py`; a DIFFERENT scope and number from [domain-rules.md](domain-rules.md#playbook-matching-layer-c)'s Layer C manual-portfolio-scan cap, #773): $10,000 account basis, ≤50% capital deployed, ≤2.5% max loss per trade, ≤4 concurrent positions with at most 2 sharing strategy-type-and-expiry (raised to 8 positions by [ADR-0009](#adr-0009--accelerated-experiment-matrix)). Paper runs a **multi-book lab**: a set of virtual $10K books in one paper account, each an experiment arm (playbook mixes, regime-engine variants, SPY vs XSP); the Live Gate attaches to the specific book configuration going live. (Book count and matrix grew from the original plan via [ADR-0009](#adr-0009--accelerated-experiment-matrix) and its amendments; `backend/seeds.py` is the current source of truth.) The **No-Stock Mandate** ([CONTEXT.md](../CONTEXT.md)) is enforced structurally where possible (cash-settled European-style underlyings, e.g. XSP) and by a same-day assignment-response rule as defense-in-depth. **Trading-mode isolation:** separate PAPER/LIVE database files; mode fixed at process start, stamped inside the DB, refused on mismatch; live DB backed up before any migration.
...
**Amendment** (2026-09-01, #927). "Halts latch — resuming requires the console" gets one narrow, mechanically-scoped carve-out: `anomaly.py`'s post-session sweep may now lift a HALT_ENTRIES it set itself (`actor="anomaly"`) back to ACTIVE, without an operator, when every rule that has contributed to it since it was last ACTIVE is (a) in a fixed self-clearable set — `REPEATED_REJECTION`, `ENVELOPE_BREACH_POSTHOC`, `PREVIEW_INFRA_FAILURE` — each admitted on one of two branches, not a single uniform criterion: `REPEATED_REJECTION` and `ENVELOPE_BREACH_POSTHOC` re-derive their evidence from scratch every sweep (a calendar window, a standing condition on open positions), so a clean recompute is trustworthy on its own terms; `PREVIEW_INFRA_FAILURE` has nothing to re-derive — it is same-night-only, no trailing window at all — but qualifies on the other branch instead, because the condition it detects (a gateway outage) is itself point-in-time transient, so last night's outage was never evidence about tonight's gateway to begin with — and (b) cleanly re-evaluated clean this same sweep (a rule the sweep skipped, or a book whose open positions were partly excluded by the envelope's era filter after a config_hash rotation, blocks the clear exactly like live evidence would). Provenance is read from the audit ledger — every halting-rule event recorded against the scope since its last ACTIVE transition — not from the halt's `reason` text, so a scope with more than one contributing rule only lifts once ALL of them clear together. `DUPLICATE_ORDER`, `ZOMBIE_FILL`, `PNL_SHOCK`, any operator-set halt (`actor="console"`), and any ntfy-set halt (`actor="ntfy"`) are excluded from this carve-out and never auto-lift — only the console can resume those, unchanged from the original decision. `FLATTEN_REQUESTED` is never touched by self-clear (decision (2) stands: automatic responses stop at HALT_ENTRIES, and this only ever moves a HALT_ENTRIES row toward ACTIVE, never downgrades a more severe state). Full mechanism in [supervision.md](supervision.md) §6.2 and `backend/anomaly.py`'s `_self_clear_expired_halts`.

**Amendment** (2026-09-02, #960). Decision (2)'s *rationale* is amended; decision (2) itself stands. The sentence "the nightly cadence plus defined-risk structure means positions are never unattended intraday" was refuted on 2026-09-02. B07's XSP 753/750 bull put took a DAY profit-target close at 1.18 placed 2026-09-01 22:45Z with the position **+$157**; it worked the whole 9/2 session unfilled and expired at the close; by 9/2 22:45Z the position was **−$175 against a $78 loss limit (2.2×)** and the run placed a loss-limit close at a 2.46 debit. Nothing looked at the position for the 17 hours in between — "defined-risk" bounded the *maximum* loss, but the nightly cadence let a winner become a 2.2× loser with no exit management in between, which is exactly the exposure the rationale claimed could not exist. **The cadence is therefore split: entries stay nightly; exits run nightly AND at 12:30 ET on weekdays** (`backend/midday_exits.py`, `pixi run midday-exits`, `scripts/register-midday-exits-task.ps1`, [supervision.md](supervision.md#midday-exit-pass)). The midday pass is exits-only — it runs the nightly run's own `_layer_a_closes` (profit target / loss limit / time rule / assignment defense) against 12:30 quotes, and re-prices a resting unfilled DAY exit to the current mid at the **same** ladder rung — under every guard the nightly run applies, and it halts outright on any reconciliation drift. What this amendment does **not** change: automatic responses still stop at HALT_ENTRIES (the midday pass never latches or clears control state and never flattens), FLATTEN_REQUESTED is still human-initiated and still rides the nightly ladder ([ADR-0011](#adr-0011--flatten_requested-closes-everything-in-scope-at-the-next-run) untouched), rung *escalation* is still one rung per evening, the remote channel is still HALT-only, and entries are still nightly. "Positions are never unattended intraday" is retired as a justification for anything: where the reasoning here or in [supervision.md](supervision.md) leaned on it, the midday pass is now the mechanism that makes intraday exit management true, rather than an assumption that it already was.
