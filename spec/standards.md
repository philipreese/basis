...
- **Git hooks** (`pixi run install-hooks`, once per worktree; #988, #997): `pre-commit` runs `scripts/verify-project.ps1 -StagedOnly`, which is `pixi run lint` scoped to the staged diff and nothing slower. `pre-push` runs `-PrePush`, which is `pixi run test-backend` (and `test-frontend` when the pushed commits touch `frontend/`) and secrets scan scoped to the commits being pushed, with redundant warning-only workflow checks skipped while the blocking main/master branch guard still runs (#997), falling back to full unscoped checks when no base can be derived. A failing test is caught at push, then again by CI; a lint error is caught at commit. `pixi run verify-hook-selftest` pins this against a throwaway repo with a fake `pixi` shim, no network.
