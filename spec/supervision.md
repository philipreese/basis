...
On a later sweep, a finding whose rule, evidence identity (exact set match), and every current sub-breach ratio all still fall within the acknowledged snapshot (`anomaly._ack_matches`) is recorded in the audit ledger as always, but does **not** re-latch — a lightweight `ANOMALY_ACK_HELD` event (same urgent-push tier as `CONTROL_STATE_CHANGED`, not that event type itself, since the row's state isn't moving) carries "acknowledged since \<date\>: \<identity\>" into the digest every night the ack holds. "Materially larger" reuses the exact re-alert band `_should_alert` uses for push dedup (`ALERT_INCREASE_THRESHOLD`, currently 10% — #925 is expected to change this band for integer counts; both callers share one predicate, `_within_alert_band`, so that change lands once). A different breaching position, a materially larger breach, or a different rule firing on the same scope all fail the match and halt normally — and a fresh halt always clears any prior ack on the row as a side effect (`set_control` sets the four ack columns unconditionally on every call, including plain ack-less resumes).
