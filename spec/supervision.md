...
**Self-clear (#927):** a HALT_ENTRIES that `anomaly.py` itself set (`actor="anomaly"`) can lift back to ACTIVE automatically, but only for REPEATED_REJECTION, ENVELOPE_BREACH_POSTHOC, and PREVIEW_INFRA_FAILURE — the rules that re-derive their evidence from scratch every sweep (a calendar window, a standing condition on open positions, or same-night gateway state) rather than merely going quiet because the run window moved on. Provenance is read from the audit ledger, not the halt's `reason` text: every halting rule that has touched the scope since it was last ACTIVE must be self-clearable AND cleanly re-evaluated this sweep before the scope lifts — one still-standing rule (e.g. a ZOMBIE_FILL that fired after the halt) blocks the clear even once the original rule ages out. DUPLICATE_ORDER, ZOMBIE_FILL, PNL_SHOCK, operator halts, and FLATTEN_REQUESTED never auto-lift; only the console can resume those.
