...
- **Stress episode observed** (`stress_episode_ok`, `stress_episode_check`), in the shape ratified in #738 — **episode × meaningful deployment**. An *episode* is an `index_history` date inside the window with a VIX close ≥ 25 or a SPY close-to-close drawdown ≥ 5% from the **window's** running peak (the peak restarts at the window start; a pre-window high cannot manufacture an in-window episode). The bare "a position was open that day" overlap is surfaced (`episode_while_position_open`) but is **not the bar** — held ≠ exposed. The row passes when on at least one episode date the book's dollars at risk (Σ `max_loss` × contracts × 100 over positions held that date, a position counting from `entry_date` through its post-mortem `exit_date`, else its expiration date if closed, else today) were **≥ 50%** (`STRESS_DEPLOYMENT_FRACTION`, pre-registered in the ADR-0010 amendment) of the book's **normal gate-window deployment** — the mean of that same daily figure over every `index_history` date in the window, flat days included — and strictly positive. A fully-deployed book that stayed calm through the episode has passed a stress test; a near-flat book has not taken it; a calm window is an unfinished sample whatever its length. Supporting numbers: peak VIX close and deepest SPY drawdown in the window, episode-date count, deployment on the best-covered episode date vs normal, and the book's **max adverse excursion** in `book_mtm_history` marks during the episode (last pre-episode mark − lowest episode mark; informational, never gating, composes with the #717 tail row).
- **Beats the SPY benchmark** (`benchmark_ok`, `benchmark_check`): the book's realized P&L on closed era trades as a return on its basis must **exceed** the SPY price return between the first and last SPY closes inside the window (`backend/benchmark.py`'s `spy_window_return`, the same definition the digest's benchmark line uses; dividends excluded). Fail-closed: no closed trades, or fewer than two SPY closes in the window, renders `fail` with the reason in the row's detail — never a silent pass.
