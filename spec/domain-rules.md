...
**VRP entry gate** (`min_vrp`, #1035): a playbook carrying this filter refuses an entry whose **VIX − SPY RV20** is below the floor, seeded at `VRP_FULL_EDGE` (2.0 vol points) on every credit seller and unset on every buyer. RV20 comes from the persisted `market_state.spy_rv20`, written nightly by `refresh_market_state` from the same closes and the same `realized_vol_20d` V2 already runs — the gate never recomputes it or reaches into the regime race. **Fail closed**: no RV20 recorded (0.0 — a fresh database, or a run of failed index fetches) holds entries and says so, rather than reading VIX − 0 as the richest premium ever recorded. **B16 is the control**: it overrides `entry_filters.min_vrp` to null, so B16 vs the B01 baseline measures whether the gate earns its keep. **B30** (AAPL earnings) has no volatility gate at all — VRP is a SPY-wide measure and says nothing about a single name, and its substantive gate is `require_scoped_catalyst`, which fires only inside an AAPL earnings window. A true per-underlying implied-vol gate needs an ATM IV history the system has never collected.
