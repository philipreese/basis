...
| ~~IVR GATE (INCOME)~~ | **Removed (#1035.)** It read `underlying_ivrs` — an RV20 percentile rank, never an implied-vol rank — so it demanded the market already be moving before anyone could sell premium, which is close to backwards. Its replacement is the per-playbook `min_vrp` entry filter below: same intent, asked of the quantity that answers it. |
| IVR GATE (DEBIT) | rank > 70: suppress naked long options, show spreads only. **Survives** #1035: read against the realized-vol rank this still says the true thing, because a market that has been moving hard really does have expensive options. Only the floor inverted. |

**Executor book scans** run in `book_mode`: the DIRECTIONAL and UNDERLYING CONCENTRATION gates are skipped, because a lab book ladders multiple positions on one underlying by design — its concentration policy is the **lab book envelope cap** (`max_positions`, `max_same_strategy_expiry`, `max_deployed_pct` in [backend/book_gates.py](../backend/book_gates.py)'s `Envelope`), not the manual portfolio scan's MAX POSITIONS/MAX CAPITAL rows above. The manual console keeps all gates. The DEBIT ceiling can be disabled per book (`ignore_ivr`); no seeded book uses it since #1035 repurposed B16.

**VRP entry gate** (`min_vrp`, #1035): a playbook carrying this filter refuses an entry whose **VIX − SPY RV20** is below the floor, seeded at `VRP_FULL_EDGE` (2.0 vol points) on every credit seller and unset on every buyer. RV20 comes from the persisted `market_state.spy_rv20`, written nightly by `refresh_market_state` from the same closes and the same `realized_vol_20d` V2 already runs — the gate never recomputes it or reaches into the regime race. **Fail closed**: no RV20 recorded (0.0 — a fresh database, or a run of failed index fetches) holds entries and says so, rather than reading VIX − 0 as the richest premium ever recorded. **B16 is the control**: it overrides `entry_filters.min_vrp` to null, so B16 vs the B01 baseline measures whether the gate earns its keep. **The gate applies only to SPY-proxied underlyings** (`telemetry_key(ticker) == "SPY"`, so SPY and XSP), and the check enforces that rather than leaving it to the seeds: `spy_rv20` is SPY's realized vol and `vix_close` is SPY's implied vol, so their difference describes the S&P and nothing else. **B09** (IWM), **B10** (GLD), **B22** (TLT) and **B30** (AAPL earnings) therefore carry no volatility floor at all — B09/B10/B22 inherit these same seeded credit sellers (they whitelist no playbooks) and would otherwise gate a gold or bond spread on the equity market's premium. For B30 the substantive gate is `require_scoped_catalyst`, which fires only inside an AAPL earnings window; for the other three there is no replacement, and that is a stated gap, not an oversight. A real per-underlying volatility gate needs an ATM implied-vol history the system has never collected.

**An absent rank is refused, not read as zero.** `automated_ivrs` DROPS a symbol it cannot rank (fewer than 60 closes) rather than keeping it stale, on the documented promise that its windowed playbooks stay ineligible. That promise held only while some floor sat above zero; with every floor at 0.0 an unrankable underlying would pass `0.0 <= 0.0 <= 100` — and clear the rank>70 ceiling — on telemetry that does not exist. `check_entry_filters` refuses the absence explicitly instead. A rank that genuinely reads 0 is a real measurement and passes.
