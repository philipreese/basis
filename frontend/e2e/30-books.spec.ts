...
test('Books tab renders the lab book matrix with the Live Gate checklist', async ({ page }) => {
  ...
  // init_db seeds the complete ADR-0009 experiment matrix (31 books after
  // the #219 sweeps, #254 regime-flip exit, #316 consensus arm, #317
  // earnings arm, and #318 roll arm); B00 legacy is excluded.
  await expect(table.locator('tbody tr')).toHaveCount(31);
  await expect(table).toContainText('B01');
  await expect(table).toContainText('B31');
