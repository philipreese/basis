...
<article class="carbon-card overflow-hidden transition
  {postMortem.outcome === 'WIN' ? 'border-emerald-500 dark:border-emerald-600 dark:glow-emerald' :
   postMortem.outcome === 'LOSS' ? 'border-rose-500 dark:border-rose-600 dark:glow-rose' :
   'border-slate-200 dark:border-slate-800'}">

  <div class="p-5 border-b
    {postMortem.outcome === 'WIN' ? 'bg-emerald-50/40 dark:bg-emerald-950/15 border-emerald-100 dark:border-emerald-900/10' :
     postMortem.outcome === 'LOSS' ? 'bg-rose-50/40 dark:bg-rose-950/15 border-rose-100 dark:border-rose-900/10' :
    ...
    <div class="flex justify-between items-start">
      ...
      <span class="px-2.5 py-1 text-xs font-black rounded uppercase tracking-wider
  ...
  <div class="p-5 space-y-4">
    <div class="grid grid-cols-3 gap-3">
      <div>
        ...
        <span class="text-base font-black carbon-mono {postMortem.realized_pnl >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}">
      ...
      <div>
        ...
        <span class="text-base font-black carbon-mono dark:text-white">
    ...
    {#if postMortem.lesson_tags.length > 0}
      <div class="flex flex-wrap gap-2">
        {#each postMortem.lesson_tags as tag}
          <span class="px-2 py-0.5 text-[10px] font-semibold rounded bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300">{tag}</span>
        {/each}
      </div>
    {/if}

    {#if postMortem.user_override_logged}
      <span class="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300">
        ⚠ Override logged
      </span>
    {/if}
  </div>
