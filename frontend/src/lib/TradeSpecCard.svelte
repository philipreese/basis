...
<section class="mb-8">
  ...
  {#if result.hard_blocks.length > 0}
    <div class="mb-5 rounded border border-rose-400 bg-rose-50 dark:bg-rose-950/20 dark:border-rose-900/50 p-5">
      ...
      <div class="space-y-2">
        {#each result.hard_blocks as block}
          <div class="flex items-start gap-3 text-xs text-rose-800 dark:text-rose-300">
            <span class="carbon-mono font-bold text-[10px] px-1.5 py-0.5 bg-rose-100 dark:bg-rose-950 rounded shrink-0">{block.check}</span>
  ...
  {#if result.warnings.length > 0}
    <div class="mb-5 space-y-3">
      {#each result.warnings as warning}
        ...
        <div class="rounded border {confirmed ? 'border-slate-200 dark:border-slate-800 opacity-60' : 'border-amber-300 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-950/10'} p-4 text-xs flex items-start justify-between gap-4">
          <div>
            <span class="carbon-mono font-bold text-[10px] px-1.5 py-0.5 rounded {confirmed ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-400'} mr-2">{warning.check}</span>
          ...
          {#if !confirmed}
            <button
              ...
              class="shrink-0 px-3 py-1.5 text-[10px] font-black rounded bg-amber-500 hover:bg-amber-600 text-white uppercase tracking-wider cursor-pointer transition"
            >
              Acknowledged
            </button>
          {:else}
  ...
  {#if result.spec && result.hard_blocks.length === 0}
    ...
    <div class="carbon-card dark:bg-slate-950/80 dark:backdrop-blur-md overflow-hidden shadow-sm">
      ...
      <div class="px-6 py-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
        <div class="flex flex-wrap gap-3 items-center justify-between">
          ...
          <div class="text-right">
            ...
            <p class="text-2xl font-black carbon-mono text-indigo-650 dark:text-indigo-400">{formatDollar(spec.limit_price_per_share)}</p>
          </div>
        </div>
      </div>
      ...
      <div class="p-6 space-y-6">
        ...
        <div>
          ...
          <div class="flex gap-3 flex-wrap">
            {#each spec.break_even_prices as be}
              <span class="px-3 py-1 rounded bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 carbon-mono text-sm font-bold dark:text-white">{formatDollar(be)}</span>
            {/each}
          </div>
        </div>
        ...
        <div class="rounded border border-indigo-200 dark:border-indigo-900/40 bg-indigo-50/50 dark:bg-indigo-950/10 p-4">
          <span class="text-[10px] font-bold text-indigo-655 dark:text-indigo-400 uppercase tracking-wider block mb-1.5">GTC Closing Order Instructions</span>
          <p class="text-xs text-slate-650 dark:text-slate-300 leading-relaxed">{spec.closing_order_instructions}</p>
        </div>
        ...
        {#if canProceed}
          {#if !showJournalForm}
            <div class="flex justify-end pt-2">
              <button
                ...
                class="px-6 py-3 text-sm font-black rounded bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer transition shadow-md uppercase tracking-wider"
              >
          ...
          {:else}
            ...
            <div class="carbon-card p-5 space-y-4 border-emerald-250 dark:border-emerald-900/45 dark:bg-emerald-950/10">
              ...
              <label class="block text-xs font-semibold text-slate-500">
                ...
                <textarea
                  ...
                  class="w-full mt-1 px-3 py-2 border rounded dark:bg-slate-900 dark:border-slate-800 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-emerald-400"
                ></textarea>
              </label>
              ...
              <label class="block text-xs font-semibold text-slate-500">
                ...
                <input
                  ...
                  placeholder="e.g. 2.5"
                  class="w-full mt-1 px-3 py-2 border rounded dark:bg-slate-900 dark:border-slate-800 text-sm carbon-mono focus:outline-none focus:ring-2 focus:ring-emerald-400"
                />
              </label>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <label class="block text-xs font-semibold text-slate-500">
                  ...
                  <select
                    ...
                    class="w-full mt-1 px-3 py-2 border rounded dark:bg-slate-900 dark:border-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400"
                  >
                  ...
                  <div class="flex gap-2 mt-2">
                    {#each [1, 2, 3, 4, 5] as n}
                      <button
                        ...
                        class="w-9 h-9 rounded text-sm font-black border transition cursor-pointer {confidenceRating === n ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-500 hover:border-emerald-400'}"
                      >
                        {n}
                      </button>
                    {/each}
              ...
              <div class="flex justify-between items-center pt-2">
                ...
                <button
                  ...
                  class="px-6 py-3 text-sm font-black rounded bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 disabled:cursor-not-allowed text-white cursor-pointer transition shadow-md uppercase tracking-wider"
                >
