...
*   **Issue-Driven Workflow**: Work items are tracked as GitHub issues; the project board is the source of truth for what is open and done. Branch from an issue with `gh issue develop <n> --checkout` and include `Closes #<n>` in the PR body so merging auto-closes the issue and moves the board card. Create an issue (`gh issue create`) before starting work that doesn't have one. Full loop in [`spec/standards.md`](spec/standards.md) → "Issue & PR workflow".
