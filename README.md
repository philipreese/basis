...
- **Anomaly rules** (`anomaly.py`): repeated rejections, duplicate orders, P&L shocks, and post-hoc envelope breaches auto-halt their scope. A standing breach on an already-open position re-fires every run — the ledger records every occurrence, but the urgent ntfy push dedupes: first occurrence alerts, unchanged repeats fold into the regular digest line, and a magnitude increase past +10% of the last-alerted value re-alerts (state persisted in `anomaly_alert_state`, #922).
