...
- **Supervision console**: an installable web app (PWA) for watching and steering the executor — status strip, lab-book comparison with the Live Gate checklist, reconciliation resolution, audit trail, kill switches — plus a diagnostic playbook scan and position views (Overview · Scan · Books · Analysis · Settings; no session gating, #315).
...
│   ├── digest.py                  Executor digest (human ntfy push + dense log lines) + urgent tiering
...
| `pixi run midday-exits` | Run the 12:30 exits-only pass once (fires and re-prices exits; no entries) |
...
**Gateway lifecycle** (`gateway_lifecycle.py`): the nightly run starts IB Gateway on demand through IBC, polls the API port, runs the pipeline, and kills the Gateway process tree — no 24/7 session (resting GTC profit-takers live server-side at IBKR). On market holidays (`calendars.py`) the executor writes its heartbeat and exits without launching Gateway. After each trading-day run the database is copied to `DB_BACKUP_DIR` (default `~/OneDrive/basis-db-backups`, 7 rotations) — once real fills land the DB is Live Gate evidence, and a failed backup pushes an ntfy alert (`db_backup.py`). Every snapshot is verified before it's trusted (#649): opened and checked for a minimum set of tables (`books`, `orders`, `positions`, `audit_events`) that are non-empty whenever the source's are, and compared against the newest existing rotation's size — a snapshot under 25% of that size is quarantined under a `.suspect` suffix rather than risk overwriting a good backup. Either failure mode refuses the snapshot, deletes/quarantines it, and pushes an urgent `SCHEDULER_ALERT` instead of rotating in a silently-empty file. A Layer C order-path broker error is audited and skips that book; later books continue under the existing control checks and run-level anomaly halt rules. Each book without a submitted entry gets an `ENTRY_NOT_TAKEN` audit row explaining its deepest refusal stage; see [supervision](spec/supervision.md#entry-funnel-audit). One-time setup: `scripts/setup-ibc.ps1` writes the bot's paper credentials into IBC's **local** `config.ini` (never this repo or `.env`); set `IBC_START_SCRIPT` in `.env`; schedule with `scripts/register-executor-task.ps1` (weekdays 6:45 PM local). Manual run: `pixi run executor-nightly`. A read-only morning fill check (`fill_check.py`, `scripts/register-fill-check-task.ps1`, 10:00 weekdays) pushes which resting orders filled at the open — notification only; it never places, cancels, or books anything. Since #960 the evening pipeline is no longer the only pass that places orders: the 12:30 midday exit pass (`midday_exits.py`, below) fires and re-prices **exits**. Entries, rolls, ladder escalation, fill→position booking, reconciliation runs, and the executor heartbeat all remain evening-only.
...
- **Kill switch** (`trading_control.py`): `ACTIVE` / `HALT_ENTRIES` / `FLATTEN_REQUESTED` per scope; fail-closed defaults (missing or unreadable state reads as halted); halts latch until resumed from the console with a typed reason (the one narrow exception: `anomaly.py` may self-clear its own prior `actor="anomaly"` halt once every rule behind it has stopped finding evidence, #927); a sentinel `HALT` file overrides everything; the remote ntfy channel accepts HALT only — RESUME over it is ignored and audited. A RESUME may also acknowledge a specific anomaly finding (`ack: {"rule": "..."}`, #931) — the same evidence remaining true on the next sweep no longer re-latches the halt; a materially larger breach, a different position, or a different rule still does, and the acknowledgment clears itself once the underlying evidence resolves.
- **Stand-down line** (`digest.py`): when a run places nothing and every book's `ENTRY_NOT_TAKEN` row shares one stage, the digest says so in one line — naming the refusing regime when every reason is the same regime gate, plus a streak count from the second consecutive session. A lab-wide stop (an `EVENT_CATALYST` night permits only the long-vol structures, which ship disabled) used to render identically to a quiet night. Reporting only; no entry decision reads it.
...
- **Digest + dead-man watchdog** (`digest.py`, `scripts/watchdog.ps1`): fixed-order nightly digest (a halted system says so first, every night), rendered twice from one data model — the readable body pushed to ntfy (a leading sentence with fleet counts, the regime consensus in words and the one operator action; words beside every fraction; blocked positions named by what they are; idle books as a count plus reason; a Live Gate horizon; fitted under ntfy's 4,096-byte limit) and the dense grep-friendly log line the executor logs and persists beside it; interrupt-worthy events go out as a separate urgent push; an independent Scheduled Task (`scripts/register-watchdog-task.ps1`) alerts if the executor's heartbeat goes stale. When the broker session fails to open, API errors captured during the connect attempt ride in the `EXECUTOR_BROKER_UNAVAILABLE` audit payload, and a known needs-a-human code (e.g. IBKR 10141, paper-trading disclaimer not accepted) turns the digest's broker-unavailable line into the specific operator instruction.
- **Afternoon preflight** (`preflight.py`): a 14:00 report-only rehearsal of the nightly run's broker machinery — the 18:45 run is otherwise the system's only full rehearsal, so every connect-path or drift failure used to cost a full day. Preflight launches Gateway and opens the broker session exactly as the executor does (including the connect retry and the needs-a-human error classification above), runs the executor's broker-vs-books comparison (`reconciliation.compare_books`) read-only, prices a near-the-money XSP put vertical from live quotes and runs it through `preview_spread` (whatIfOrder only — `PreviewRejectedError` is itself a reported finding), and reports any non-ACTIVE control scope and heartbeat staleness against a preflight-specific bar (the previous trading day's evening run — not "as of now", which at 14:00 is already today and would flag every healthy day). Drift explainable by a live STAGED/SUBMITTED order on the same legs (a TP, entry, or close fill the evening sync hasn't booked yet) is reported informationally, not counted toward the problem count and without the resolve-via-panel instruction, since resolving it now would double-book on top of the sync; only drift with no such explanation stays actionable. Each step is independently guarded — an exception becomes a finding, never a crash — and the run ends in one ntfy push ("basis preflight: all clear" / "basis preflight: N problem(s)") plus a `PREFLIGHT_RUN` audit event, its only database write; if that final push itself fails after retries, the run exits nonzero rather than reporting success with the report undelivered. It never places or cancels orders, never mutates books/positions/control state, never writes reconciliation runs or the executor heartbeat, and takes its own `preflight` Gateway-tenant lock (skipping cleanly when the executor or any other tenant is live). Schedule with `scripts/register-preflight-task.ps1` (14:00 weekdays); manual run: `pixi run preflight`.
- **Midday exit pass** (`midday_exits.py`): a 12:30 exits-only executor run (#960). On 2026-09-02 a B07 XSP bull put sat on an unfilled profit-target close all session (+$157 when it was placed) and was −$175 against a $78 loss limit — 2.2× — by the time the nightly run looked again 17 hours later. The pass re-prices any resting DAY exit that is still unfilled at the broker (cancel-and-confirm, then re-issue at the **same** ladder rung re-marked to the current mid — rung escalation stays nightly per ADR-0011, enforced by `MIDDAY_REPRICE_OF_KEY` on the replacement order) and runs the nightly run's own `_layer_a_closes` for profit-target/loss-limit/time-rule closes against 12:30 quotes. Every nightly guard applies: own `midday_exits` Gateway-tenant lock, a 09:45–15:45 ET window (the task's `-StartWhenAvailable` can otherwise fire the pass after the close), `reconciliation.compare_books` first (**unexplained** drift halts the pass outright and nothing is placed — stricter than the nightly run, because no operator is adjudicating at 12:30; drift riding on a live `STAGED`/`SUBMITTED` order's legs is the evening sync's own work in flight and does not halt, but its legs are still never traded — `reconciliation.drift_is_sync_pending`, one predicate shared with preflight, and the only reason the pass survives a morning on which an entry filled), the stale-mark guard, the ladder cap, the PARTIAL latch, and the pending-close skip. HALT_ENTRIES does not block exits; FLATTEN_REQUESTED is left to the nightly ladder; a close that already filled is left to the evening sync untouched. It places no entries, rolls, or TP children, writes no `reconciliation_runs` row, syncs no order states, books no fills, and never writes the executor heartbeat (the 22:00 watchdog must keep watching for the *evening* run). It pushes only when it acted, halted, or left a position needing attention — a quiet pass writes one `MIDDAY_EXITS_QUIET` audit event and nothing else, while a pass that cancelled a resting exit and could not replace it pushes `urgent` and writes `MIDDAY_EXITS_DEGRADED`. Schedule with `scripts/register-midday-exits-task.ps1` (12:30 weekdays); manual run: `pixi run midday-exits`.
...
The checkout **is** the deployment. On the executor host, scheduled tasks and long-running servers execute directly out of the repository working directory (`C:\Users\pbree\source\repos\alpaca-agent-bot`). Since #1019 the console is **served by the backend**: FastAPI mounts the built `frontend/dist` at `/`, so one process serves both the API and the UI on `127.0.0.1:8000`. The `basis-console` Scheduled Task runs `pixi run server` (working directory: the checkout) and that is the whole console; `scripts/register-console-task.ps1` registers it (#1021), so it is no longer the one task created by hand. It triggers **at startup** under an S4U principal — the console must survive a reboot with nobody logged in, because it is the operator's only RESUME surface (ADR-0008) and nothing pages when it is down — with restart-on-failure and no execution time limit (the Scheduled Task default kills its action after three days, which for a server is a guaranteed outage that looks exactly like a crash). The old `basis-console-ui` task — a permanently-running Vite dev server — is retired; `.\scripts\register-console-task.ps1 -RemoveLegacyUi` deletes it, and leaving it registered means a second process still serving a stale build on :5173. `VITE_API_PROXY_TARGET`, `VITE_EXTRA_ALLOWED_HOST` and the `CORS_ORIGINS` override all become dev-only concerns, because the served console is same-origin and never proxies or preflights.

Seven Windows Scheduled Tasks run directly against the checkout:

- **`basis-console`**: `pixi run server` (the FastAPI app serving the API and the built UI) — at startup, restart-on-failure (`scripts/register-console-task.ps1`).
...
- **`basis-midday-exits`**: `pixi run midday-exits` (exits-only pass: fires and re-prices exits, no entries) — weekdays at 12:30 local (`scripts/register-midday-exits-task.ps1`).
